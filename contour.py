...
def find_upper_or_lower_curve(image, circle, line, fitting_points=20):
    ...
    for i in range(1, fitting_points):
        angle = angle_small + (-2 * math.pi - angle_small + angle_big) / fitting_points * i
        x = circle_x + circle_r * math.cos(angle)
        y = circle_y + circle_r * math.sin(angle)
        distance, index = kd_tree_finder.query([x, y], k=1)
        nearest_point = edge_points[index]
    ...
    if totlen1 < totlen2:
        ...
        return angle_small / math.pi * 180, angle_big / math.pi * 180 - 360, angle1, angle2
...
image = cv2.imread('good.jpg', cv2.IMREAD_COLOR)
...
detected_circles = cv2.HoughCircles(edges,  
                   cv2.HOUGH_GRADIENT, 1, 200000, param1 = 20, 
               param2 = 8, minRadius = 120, maxRadius = 200) 
...
angle_s, angle_e, angle1, angle2 = find_upper_or_lower_curve(edges, detected_circles[0][0], lines[0][0])
cv2.ellipse(image, (int(circle[0]), int(circle[1])), (int(circle[2]), int(circle[2])), 0, angle_s, angle_e, (0, 255, 0), 2)
cv2.circle(image, (int(x1), int(y1)), 5, (0, 0, 255), 3)
cv2.circle(image, (int(x2), int(y2)), 5, (0, 0, 255), 3)
cv2.line(image, (int(x1 - math.sin(angle1) * tangent_line_length), int(y1 + math.cos(angle1) * tangent_line_length)), (int(x1 + math.sin(angle1) * tangent_line_length), int(y1 - math.cos(angle1) * tangent_line_length)), (0, 0, 255), 2)
cv2.line(image, (int(x2 - math.sin(angle2) * tangent_line_length), int(y2 + math.cos(angle2) * tangent_line_length)), (int(x2 + math.sin(angle2) * tangent_line_length), int(y2 - math.cos(angle2) * tangent_line_length)), (0, 0, 255), 2)

angle_drop_line = abs(math.atan((math.tan(line[1]) - math.tan(angle1)) / (math.tan(line[1]) * math.tan(angle1) + 1)))
if abs(angle1 - angle2) > math.pi:
    angle_drop_line = math.pi - angle_drop_line
print('The angle between the line and the tangent line is: {} deg'.format(angle_drop_line / math.pi * 180))
